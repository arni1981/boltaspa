...
/* NEW: Aggregate isolated scores by stage name independently */
stage_scores AS (
  SELECT lc.id AS league_competition_id,
         p.user_id,
         m.stage,
...
ranked AS (
  SELECT up.league_competition_id,
         ...
         COALESCE(sfa.full_stage_array, '{}'::jsonb) AS full_stage_array, -- NEW DATA ENTRY
         rank() OVER w AS rank,
    ...
    LEFT JOIN stage_form_agg sfa ON sfa.league_competition_id = up.league_competition_id AND sfa.user_id = up.user_id -- JOIN STAGE ARRAY
...
SELECT ranked.league_competition_id,
       ...
       ranked.full_stage_array, -- EXPOSE TO RAILS
       ranked.rank,
       ranked.prev_points,
       ranked.next_points,
