Rollbar.configure do |config|
  ...
  config.access_token = Rails.application.credentials.option('ROLLBAR_ACCESS_TOKEN'.downcase)

  config.enabled = Rails.env.production? && Rails.application.credentials.option('ROLLBAR_ACCESS_TOKEN'.downcase).present?

  # By default, Rollbar will try to call the `Current.user` controller method
  # to fetch the logged-in user object, and then call that object's `id`
  # method to fetch this property. To customize:
  # config.person_method = "my_Current.user"
